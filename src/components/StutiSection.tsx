...
import gangeshanandaWriter from "@/assets/stuti-gangeshananda-new.jpg";
...
const aartiVerses = [
  { lines: ["आरती सदगुरु सुबोधानंदा, मंगल मूरती आनंदकंदा ।", "आरती सदगुरु की ।।"] },
  { lines: ["भाल विभूति छटा शुभ निर्मल, प्रेम सुधार्णव निरुपम निष्कल,", "कौशेय वसन स्मित मुख चंदा, आरती सदगुरु सुबोधानंदा ।", "आरती सदगुरु की ।।"] },
  { lines: ["ब्रह्म उपनिषद् गीता के ज्ञाता, सिद्धबाड़ी आश्रम मन राता,", "गुरुदेव आपके श्री चिन्मयानंदा, आरती सदगुरु सुबोधानंदा ।", "आरती सदगुरु की ।।"] },
  { lines: ["शिव स्वरूप शंकर के सेवक, भक्त वृंद शिष्यों के खेवक,", "बही अजस्र ज्ञान की गंगा, आरती सदगुरु सुबोधानंदा ।", "आरती सदगुरु की ।।"] },
  { lines: ["करुणा सागर साधक हितकर, शास्त्र प्रमाण ज्ञान यज्ञ रुचिकर,", "मूर्तिमंत मानहु श्रुति छंदा, आरती सदगुरु सुबोधानंदा ।", "आरती सदगुरु की ।।"] },
  { lines: ["देखि हिमालय गंग तट रीझे, कृपा दृष्टि अब गुरुवर कीजे,", "शरणागत हूँ ज्ञान अखंडा, आरती सदगुरु सुबोधानंदा ।", "आरती सदगुरु की ।।"] },
];
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
      ...
      <div className="grid grid-cols-1 gap-1">
        {visibleNames.map((name, index) => (
          <motion.div key={index} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
            ...
            <span className="font-body text-muted-foreground text-xs min-w-[2.5rem]">{String(index + 1).padStart(3, "0")}.</span>
            <span className="font-body text-foreground text-sm leading-relaxed">{name}</span>
          </motion.div>
        ))}
      </div>
      <button onClick={() => setIsExpanded(!isExpanded)}
        ...
        {isExpanded ? (<><span>Show less</span><ChevronUp size={14} /></>) : (<><span>Read more ({stutiNames.length - 12} more names)</span><ChevronDown size={14} /></>)}
      </button>
    </motion.div>
  );
};
      ...
      {aartiVerses.map((verse, vi) => (
        <div key={vi} className="border-b border-gold/10 last:border-0 pb-4 last:pb-0">
          {verse.lines.map((line, li) => (
            <p key={li} className={`font-body text-sm leading-relaxed text-center ${line.startsWith("आरती सदगुरु की") ? "text-primary font-medium mt-1" : "text-foreground"}`}>{line}</p>
          ))}
        </div>
      ))}
    </div>
    <p className="text-right text-xs text-muted-foreground mt-6 italic">~ डॉ॰ सीमा मिश्रा, कानपुर</p>
  </motion.div>
);
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
      ...
      <div className="space-y-4">
        {visibleStanzas.map((stanza, index) => (
          <div key={index} className="border-b border-gold/10 last:border-0 pb-4 last:pb-0">
            <p className="font-body text-sm leading-7 text-center whitespace-pre-line text-foreground">{stanza}</p>
          </div>
        ))}
      </div>
      <button onClick={() => setIsExpanded(!isExpanded)}
        className="mt-5 mx-auto flex items-center gap-2 text-xs font-body text-primary hover:text-primary/80 transition-colors px-4 py-2 rounded-lg border border-gold/20 hover:border-gold/40">
        {isExpanded ? (<><span>Show less</span><ChevronUp size={14} /></>) : (<><span>Read more ({shraddhanjaliStanzas.length - 6} more sections)</span><ChevronDown size={14} /></>)}
      </button>
    </motion.div>
  );
};
...
const gangeshanandaVerses = [
  [
    "शिवं शान्तं शुद्धं सुमतिदातारं भ्रम हरं ",
    "परं पुण्यं अनघं, पदमपत्रमिव स्थितम् ।",
    "विभूतिं भूतीनां परं महनीयं च महतां ",
